<?php
/** 
 * [bngtech_remove_hook description]
 * @return [type] [description]
 */
function bngtech_remove_hook() {
	...
	add_action('woocommerce_single_product_summary','bngtech_woo_rating',5);

	add_action('woocommerce_mid_shop_loop_item_title','woocommerce_template_loop_product_link_close',5);
	add_action('woocommerce_mid_shop_loop_item_title','bngtech_product_cart_button',10);
	add_action('woocommerce_shop_loop_item_title','bngtech_product_title',10);
	add_action('bngtech_woocommerce_product_thumbnails','woocommerce_show_product_thumbnails',20);
}
bngtech_remove_hook();
...
define( 'bngtech_WISHLIST_ACTIVED', in_array( 'yith-woocommerce-wishlist/init.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) );

/** 
 * [bngtech_product_title description]
 * @return [type] [description]
 */
function bngtech_product_title(){
	echo '<h4 class="woocommerce-loop-product__title pro-title"><a href="'.get_the_permalink().'">' . get_the_title() . '</a></h4>';
}

/** 
 * [bngtech_product_cart_button description]
 * @return [type] [description]
 */
function bngtech_product_cart_button(){
	global $product;
	...
	print '<div class="product-action">';
		print '<a '.http_build_query($attributes,' ',' ').' class="'.$class.'" href="'.$product->add_to_cart_url().'"><i class="fas fa-shopping-cart"></i></a>';
		 print bngtech_vc_yith_wishlist();
		 print '<a href="'. get_the_permalink() .'"><i class="fas fa-search"></i></a>';
	print '</div>';
}

/** 
 * [bngtech_woo_rating description]
 * @return [type] [description]
 */


function bngtech_woo_rating(){
	global $product;
	...
	$html .= '</div>';
	print bngtech_woo_rating_html($html);
}

function bngtech_woo_rating_html($html){
	return $html;
}
 ...
 * @return [type] [description]
 */
function bngtech_woo_shop_rating(){
	global $product;
	$rating = $product->get_average_rating();
...
function bngtech_get_price(){
	ob_start(); ?> 
	<span class="price"><?php print bngtech_get_price_html(); ?></span>
	<?php
	return ob_get_clean();
}

function bngtech_get_price_html(){
	global $product;
	return $product->get_price_html();
}

/** 
 * [bngtech_comment_rating description]
 * @param  [type] $rating [description]
 * @return [type]         [description]
 */
function bngtech_comment_rating($rating){
	$review = 'Rating '.$rating.' out of 5';
	$html = '';
...
add_filter('add_to_cart_fragments', 'bngtech_woocommerce_header_add_to_cart_fragment');

/** 
 * [bngtech_woocommerce_header_add_to_cart_fragment description]
 * @param  [type] $fragments [description]
 * @return [type]            [description]
 */
function bngtech_woocommerce_header_add_to_cart_fragment( $fragments ) {
    global $woocommerce;
    ob_start();
    ?>
		<a class="cp-minicart" href="<?php echo wc_get_cart_url(); ?>"><i class="fas fa-shopping-cart"></i><span id="bngtech-cart" class="mini-cart-items"><?php print WC()->cart->get_cart_contents_count(); ?></span></a>
    <?php
    $fragments['a.cp-minicart'] = ob_get_clean();
    return $fragments;
}

function bngtech_vc_yith_wishlist( $style = 1 ) {
...
add_filter('woocommerce_product_additional_information_heading','bngtech_tab_heading');
add_filter('woocommerce_product_description_heading','bngtech_tab_heading');

/** 
 * [bngtech_tab_heading description]
 * @param  [type] $heading [description]
 * @return [type]          [description]
 */
function bngtech_tab_heading($heading){
	return '';
}

/** 
 * [bngtech_woo_pagination description]
 * @param  [type] $pagination [description]
 * @return [type]             [description]
 */
function bngtech_woo_pagination($pagination){
	$pagi = '';
	if( $pagination !=''){
