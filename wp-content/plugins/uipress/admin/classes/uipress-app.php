...
class uipress_app
{
  ...
  public function run()
  {
    ...
    //AJAX
    add_action("wp_ajax_uip_get_menu", [$this, "uip_get_menu"]);
  ...
  public function toolbar_actions_front()
  {
    ...
    add_action("wp_body_open", [$this, "build_toolbar"]);
  ...
  public function add_login_styles()
  {
    ...
    ?>
      ...
      <?php
      if ($loginBg) { ?>
      <style type="text/css"> body {  background-image:url('<?php echo $loginBg; ?>')  !important; } </style> 
      <?php }
  ...
  public function capture_wp_menu($parent_file)
  {
    ...
    //STOP WP MNEU FROM BEING PROCESSED
    //$menu = [];
    //SAVE MASTER MENEU OUT TO TRANSIENT
  ...
  <div id="uip-toolbar" class="uip-padding-s uip-border-box uip-body-font">
    ...
    <div id="uip-toolbar-content" v-if="!loading"> 
      ...
      <div v-if="!loading" class="uip-flex">
        <div class="uip-flex uip-flex-center uip-margin-right-xxs" v-if="!defaults.front && isSmallScreen()">
        ...
        <div class="uip-legacy-admin uip-flex-grow">
          <div class="uip-hidden" 
          :class="{'uip-nothidden' : showLegacy() && !isSmallScreen()}">
            <?php if (!$this->front) {
  ...
  public function output_admin_menu()
  {
	...
	<div id="uip-admin-menu" class="uip-flex uip-flex-column">
		<?php echo $this->output_loader(); ?>
	</div>
	
	<?php
  }
  /**
   * Outputs UIP menu loading placeholder
   * @since 2.2
   */
  public function output_loader()
  {
    ...
    <div v-if="loading">
      ...
      <div  class="uip-padding-left-s uip-padding-top-xs">
        ...
        <div class="uip-flex uip-flex-row uip-padding-xxs" style="
          padding-top: 0;
      ">
          <div>
            <svg class="uip-margin-right-xs" height="20" width="20">
  ...
  public function uip_get_menu()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uip-security-nonce", "security") > 0) {
      ...
      if (is_array($usergenerated) && count($usergenerated) > 0) {
        ...
        $menu["menu"] = $this->uip_get_active_menu_item($usergenerated, $menu);
        $menu["customMenu"] = "true";
      } else {
        $menu = get_transient("uip-admin-menu-" . $userid);
        $menu["customMenu"] = "false";
      }

      $returndata["menu"] = $menu;
      $returndata["prefs"] = $utils->get_user_preferences();
  ...
  public function uipress_get_updates()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uip-security-nonce", "security") > 0) {
      $utils = new uipress_util();
      $showUpdates = $utils->valid_for_user($utils->get_option("toolbar", "updates-disabled-for", true));
  ...
  public function uipress_get_notices()
  {
    if (defined("DOING_AJAX") && DOING_AJAX && check_ajax_referer("uip-security-nonce", "security") > 0) {
      ...
      $showNotifcations = $utils->valid_for_user($utils->get_option("toolbar", "notifcations-disabled-for", true));
  ...
  public function get_app_settings_options($settings, $network)
  {
    ...
    $temp["premium"] = true;
    $temp["value"] = stripslashes($utils->get_option($moduleName, $temp["optionName"]));
    $options[$temp["optionName"]] = $temp;
    ...
    $temp = [];
    $temp["name"] = __("HTML for <head>", $this->textDomain);
