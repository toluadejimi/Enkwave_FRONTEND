...
class Build_App_Online_Admin {
    ...
    public function save_new_post( $post_id ) {
        ...
        if($product && $options['send_push_on_product_publish'] && $product->get_status() == 'publish') {

            $fields = array();

            $fields['included_segments'] = array("All");

            $fields['headings'] = array("en" => trim(strip_tags($product->get_name())));
            $fields['contents'] = array("en" => trim(strip_tags($product->get_description())));
    ...
    public function send_admin_and_vendor_push_notification($thank_you_title, $order) {
        ...
        $meta_key = $options['use_fcm'] ? 'bao_fcm_token' : 'bao_onesignal_user_id';
    ...
    public function order_status_changed( $order_id ) {
        ...
        update_post_meta( $order_id, 'bao_fcm_token', sanitize_text_field($_REQUEST['fcm_token']) );
